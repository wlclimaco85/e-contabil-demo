...
as select dash1.nome, dash1.dtemit, dash1.status, dash1.quantidade, dash1.registrobase,
   ...
   select c.registrobase, p.id, p.nome, TO_DATE(to_char(b.dh_credito_liquidacao , 'dd/MM/yyyy'), 'dd/MM/yyyy') as dtemit, c.codbco as banco, 'Boletos Liquidados' as status, count(*) as quantidade
   	from boletos b
   ...
   select c.registrobase, p.id, p.nome, TO_DATE(to_char(b.data_registro_banco , 'dd/MM/yyyy'), 'dd/MM/yyyy') as dtemit, c.codbco as banco, 'Boletos Emitidos' as status, count(*) as quantidade
   	from boletos b
   	inner join contas c on c.id = b.idapibanco
   	inner join parceiros p on p.id = c.idparceiro
   	where numero_titulo_beneficiario is not null
