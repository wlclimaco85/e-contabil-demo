...
public class AlteraBoletoQueue implements Runnable {
	private BoletoProcessorFactory boletoProcessorFactory;
	private BlockingQueue<Conta> queue;

    public AlteraBoletoQueue(BlockingQueue<Conta> queue, BoletoProcessorFactory boletoProcessorFactory) {
    	this.queue = queue;
    ...
    public void run() {
        try {
            while (queue.size() > 0) {
                ...
                if (conta != null) {
                	boletoProcessorFactory.getProcessor(BancoEnum.valueById(conta.getBanco().getId())).alteraBoleto(conta);
                }
            }
