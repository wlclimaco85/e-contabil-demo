...
public class ConsultarStatusBoletoQueue implements Runnable {
    private BoletoProcessorFactory boletoProcessorFactory;
    private BlockingQueue<Conta> queue;

    public ConsultarStatusBoletoQueue(BlockingQueue<Conta> queue, BoletoProcessorFactory boletoProcessorFactory) {
        this.queue = queue;
    ...
    public void run() {
        try {
            while (queue.size() > 0) {
                ...
                if (conta != null) {
                    boletoProcessorFactory.getProcessor(BancoEnum.valueById(conta.getBanco().getId())).atualizarStatusBoleto(conta);
                }
            }
